// ============================================================
// CH-20: Glossary
// Source: docs/chapters/20-glossary.adoc
// ============================================================

#import "../lib/theme.typ": *
#import "../lib/components.typ": *

#chapter-header("20", "Glossary of Game Terms")
...
*Anchor* \
A physical object linked to a humanizing memory. Once per session, a character may interact with their Anchor to heal 1d4 Corruption. See the Corruption, Fear & Healing chapter.

*Armor Rating* \
The number of Gear dice rolled to absorb incoming damage. Each 6 rolled cancels one point of damage. See the Health, Damage & Armor chapter.
...
*Clearance Level (CL)* \
Seniority and authorization rank within the Covenant. Equals Division base CL plus Age Group modifier (Young −1, Experienced +0, Senior +1), minimum 1. Determines gear access and organizational authority. See the Character Creation chapter.

*Corruption* \
Cumulative psychological and occult damage track. Gained from pushing rolls, Division Talent activation, Fear, artifact use, and supernatural exposure. *Maximum Corruption Threshold* = 10 + Empathy score; exceeding it causes permanent catatonia and character retirement. See the Corruption, Fear & Healing chapter.
...
*Department* \
Specialization within a Division (Wayfinder, Keep, Stack). Determines the character's field expertise and Department Skill. Recovery uses Backgrounds instead. See the Divisions chapter.

*Director of Agents (DA)* \
The table's facilitator role. The DA presents scenes, adjudicates uncertainty, advances clocks, and portrays the world and opposition. See the Director of Agents Guidance chapter.

*Difficulty* \
The number of successes (6s) required to pass a skill roll. Standard range: 1 (routine) to 5+ (extreme). See the Core Resolution chapter.

*Division* \
Character class within the Verdant Covenant. Four playable Divisions: Wayfinder (research/intelligence), Recovery (field retrieval), The Keep (vault security), and Stack (logistics). See the Divisions chapter.

*Division Item* \
A signature supernatural object issued to every member of a Division. Wayfinder: Verdant Codex. Recovery: Verdant Satchel. Keep: Warden's Bracer. See the Divisions chapter.

*Division Talent* \
A class-specific ability unique to one Division. Costs +1 Corruption to activate (most talents). See the Divisions chapter.
...
*Fear Check* \
A Wits roll triggered by supernatural horror. Each 6 = success. Each 1 = +1 Corruption. No successes = failure (+1 Corruption + Panic Table roll). See the Corruption, Fear & Healing chapter.

*Fear Rating* \
Narrative classification (0–5) of how terrifying a creature or event is. Used by the DA to determine when to call for a Fear check. 0 = no check triggered. See the Corruption, Fear & Healing chapter.
...
*Push* \
Re-roll all non-6 dice after a failed roll. Costs +1 Corruption. Each roll may only be pushed once. Gear dice that show 1 on the push cause degradation. See the Core Resolution chapter.
...
*Slow Action* \
One of three action types in combat. Represents a committed effort: attacking, using a skill, administering first aid, or reloading a weapon. See the Combat chapter.
