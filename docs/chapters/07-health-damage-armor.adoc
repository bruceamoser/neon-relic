...
. *Character replacement.* A new character enters play at the start of the next Case File. They begin with the same *Total XP* as the lowest-Total-XP surviving member of the team, with *Spent XP reset to 0* — the new agent allocates their own purchases (minimum: the character creation baseline values as defined in xref:chapters/02-character-creation.adoc#chapter-creation[Character Creation]). If a character dies *mid-Case-File*, the DA may introduce the replacement at a narratively appropriate moment in the same mission — a Covenant backup team, a local contact activated as emergency support, or a specialist from HQ. See xref:chapters/13-advancement.adoc#chapter-advancement[Advancement] for the three-track XP model and spending rules.
...
A character with *Medical Training* (Keep talent) may spend a Slow Action making a *Heal (EMP)* roll (Difficulty 2) to give a dying ally *+1 to their death check die* before it's rolled (effectively making death only on a natural 1 — result 2 on the table).
...
. An ally must succeed on a *Heal (EMP) roll, Difficulty 2* (First Aid Kit required).
