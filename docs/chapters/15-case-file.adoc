...
A Neon Relic *Case File* is a reactive operations board, not a scripted sequence of set scenes. The agents work through a case over the course of *days* — typically fourteen — while the DA tracks what the world is doing in response. Each day is divided into four *shifts* (Morning, Day, Evening, Night), giving the team a fixed number of moves before the situation becomes unrecoverable.

The engine is:

* an *Operations Board* counting down days until catastrophe, with a shift row tracking time spent and organization rows tracking external pressure
* a set of *Locations* that can be approached in flexible order
* *relic milestones* keyed to specific days, representing what the anomaly does if left uncontained
...
| *Operations Board*
| The single tracking surface for the case. 14 columns count down from 14 → 1 (days until catastrophe). Contains a shift row and organization countdown rows.

| *Shift Row*
| The top row of the Operations Board. Each day-column is quartered into Morning / Day / Evening / Night. As shifts pass, quadrants are filled. Relic milestones are keyed to specific days on this row — when the DA fills a quadrant that crosses a milestone day, the artifact event triggers.
...
The first row of the board is the *shift row*. It tracks how much time the team has spent and when relic milestones fire.
...
* *Relic milestones* are keyed to specific days on this row. When the DA fills the last quadrant of a milestone day, the artifact event triggers immediately.
* At Day 1, catastrophe occurs — the relic's worst-case outcome fires.
* Relic milestone descriptions live in the Case Brief, keyed to day numbers (e.g., “Day 11: Dream-bleed begins affecting witnesses within 100 meters”).

The shift row answers three questions cleanly:
...
. *Advance consequences.* On the Operations Board, advance any organization rows triggered by time passage, loud action, public exposure, missed windows, packet interception, or artifact mishandling. Fill in the next shift quadrant on the shift row and check for relic milestones crossed this day.
...
| *Breach*
| The scene goes loud, public, or occultly unstable. An organization row advances immediately, a relic milestone may trigger on the shift row, and the next shift starts under pressure.
|===
...
Organizations are tracked as countdown rows on the Operations Board. Each organization row is an independent pressure track that shares the same 14-column grid as the shift row.
...
* The row is inert until the DA marks it active (triggered by agent action, a milestone cross-link, or a relic milestone on the shift row).
...
Entity state changes are tied to *organization milestones on the Operations Board*, not independent day numbers. NPCs gain knowledge, locations change access, and information becomes available — all triggered when the DA crosses out a milestone square on the board. This keeps the Operations Board as the single source of truth for "what changed."
...
| *Availability*
| Access condition using one of the following keywords: *open* (available from case start), *clue-locked* (requires specific information), *contact-locked* (requires an NPC introduction or referral), *time-locked* (opens on a specific day), or *packet-locked* (unlocked when a packet returns).
...
* the shift row counting down from column 14
...
| *Operations Board*
| DA
| Shift row (relic escalation track) plus Organization countdown rows. See <<operations-board>>.
...
| DA -> Players
| *Front (player-facing):* what the agents learned. *Back (DA reference):* source Locations, source NPCs, HQ fallback day, type (Containment Truth vs. supporting intel). Players physically receive cards when information is discovered — tangible progress tracker.
|===
