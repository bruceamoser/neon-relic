...
| During downtime, each character recovers *2 additional Attribute points of their choice* (beyond normal rest). One character may also attempt to clear a non-lethal Critical Injury (Heal (EMP) Difficulty 2; success removes the injury without Attribute loss).
...
| During the Equipping Phase, all agents increase their effective CL Pool by +2 (e.g., a CL 2 agent has a CL Pool of 4 for requisition purposes). This does not change the agent's personal CL for any other purpose. Items up to CL 4 may be requisitioned within this expanded pool. High-capacity weapons no longer require an above-CL roll to requisition.
...
| During downtime, one character may be treated for a non-lethal Critical Injury (removes the injury without Attribute loss if a Heal (EMP) roll at Difficulty 2 succeeds). Does not replace the Basic Medical Bay; stacks with it.
...
| Once per Case File, requisition one *item of any CL* without spending from your CL Pool and without an above-CL roll. The request takes 48 hours in-game. Item arrives at a designated dead drop; no trail. This item is in addition to your normal CL-pool allocation.
...
| *CL 3 Requisition Access*           | 5  | During Equipping Phase, the *entire cell* gains a minimum effective CL Pool of 3 — even agents whose personal CL is still 2. This does not change individual agents' personal CL score; it sets a cell-wide floor for requisition only.
...
| *CL 4 Requisition Access*           | 10 | During Equipping Phase, the entire cell gains a minimum effective CL Pool of 4 regardless of personal CL. High-tier equipment available via formal request. Does not change individual agents' personal CL scores.
...
| *Legacy Standing Bonus*             | 20 | When a cell member retires or dies, their replacement PC begins play with the retired agent's *Total XP* (lifetime earned) as their own Total and Current XP, plus 3 Standing already gained toward the cell total. Spent XP resets to 0 — the new agent allocates their own purchases.
...
The Armory's expanded CL Pool (+2) should not feel routine. The DA should occasionally impose fiction on it — the contact is busy, the shipment got held up, the fence wants something first. The mechanical benefit is persistent; the narrative texture should vary.

[[equipment-return]]
=== Equipment Return to Stack

All requisitioned items must be returned to Stack during the Aftermath Phase at the conclusion of each Case File. The return is automatic — no roll required — but the DA should note the following:

* *Consumables:* Unused consumable items return to Stack inventory. Used consumables are logged and restocked.
* *Damaged gear:* Items with degraded Gear Dice are returned in their current state. Stack repairs them between cases at no cost, but repairs may not complete before the next Equipping Phase if the cell is on a tight turnaround (less than one full downtime phase between cases).
* *Lost or destroyed items:* A single lost item carries no penalty. Multiple losses in one Case File — or a pattern of carelessness across cases — may trigger a Standing review. Stack tracks equipment by requisitioner, not by current holder.
* *Signature and owned items:* Division Items and starting kit items are permanently assigned and are never returned.
