...
[cols="1,3", frame=none, grid=rows, stripes=none]
|===
| *Case File* | A structured operation tracked on an Operations Board with a phases row (shift clock + relic milestones) and organization countdown rows. Case Files also include scenes, contacts, Containment Truths, and Covenant Request Packets. Playable in flexible order rather than through a fixed phase script. See xref:chapters/15-case-file.adoc#chapter-case-file[Case Files].
