...
[cols="1,3", frame=none, grid=rows, stripes=none]
|===
| *Case File* | A structured operation tracked on an Operations Board with a phases row (shift clock + relic milestones) and organization countdown rows. Case Files are built from Locations, NPC Cards, Information Cards, organizations, Containment Truths, and Covenant Request Packets. Playable in flexible order rather than through a fixed phase script. See xref:chapters/15-case-file.adoc#chapter-case-file[Case Files].
...
| *Director of Agents (DA)* | The table's facilitator role. The DA presents scenes, adjudicates uncertainty, tracks the Operations Board (organization rows and phases row), and portrays the world and opposition. See xref:chapters/14-gm-guidance.adoc#chapter-gm-guidance[Director of Agents Guidance].
...
| *Operations Board* | The single tracking surface for a Case File. Contains a phases row (shift clock + relic milestones) and organization countdown rows. 14 columns count down from 14 → 1 (days until catastrophe). See xref:chapters/15-case-file.adoc#chapter-case-file[Case Files].
| *Organization* | External reactive pressure during a Case File, tracked as a countdown row on the Operations Board. Uses O#M# shorthand for milestones. Examples: rival recovery operations, police inquiry, media exposure, or Church seizure attempts. An organization is not the artifact itself. See xref:chapters/15-case-file.adoc#chapter-case-file[Case Files].
|===
...
| *Phases Row* | The top row of the Operations Board. Each day-column is quartered into Morning / Day / Evening / Night. Serves as both the shift clock and the relic escalation track — relic milestones trigger when the DA fills a shift quadrant that crosses a milestone phase number. See xref:chapters/15-case-file.adoc#chapter-case-file[Case Files].
...
[cols="1,3", frame=none, grid=rows, stripes=none]
|===
| *Relic Milestones* | Phase-keyed events on the Operations Board's phases row that define what the anomaly does if it remains active, mishandled, or unresolved. When the DA fills a shift quadrant that crosses a relic milestone, the artifact event triggers. Distinct from organizations, which represent external reactive pressure. See xref:chapters/15-case-file.adoc#chapter-case-file[Case Files].
|===
