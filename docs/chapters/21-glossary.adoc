...
| *Clearance Level (CL)* | Personal equipment authorization tier (1–5). Equals Division base CL plus Age Group modifier (Young −1, Experienced +0, Senior +1), minimum 1. Your CL Pool (equal to your CL) determines how much gear you can requisition during the Equipping Phase via point-buy. CL does not control information access — that is governed by Covenant Standing. See xref:chapters/10-equipment.adoc#requisitioning-gear[Requisitioning Gear].
...
[cols="1,3", frame=none, grid=rows, stripes=none]
|===
| *Equipping Phase* | A formal between-case step where agents requisition gear from Stack using their CL Pool (point-buy: sum of item CLs ≤ personal CL). Above-CL items require a 1d6 roll (success on 6; Stack agents succeed on 5–6). All requisitioned items are returned at case end. See xref:chapters/10-equipment.adoc#requisitioning-gear[Requisitioning Gear] and xref:chapters/15-case-file.adoc#chapter-case-file[Case Files].
| *Experience Points (XP)* | The game's only advancement currency. XP are awarded at the end of each session through debrief questions and spent to raise skills, buy new talents, or increase Clearance Level. XP is tracked as three values: *Total XP* (lifetime earned), *XP Spent* (cumulative purchases), and *Current XP* (Total − Spent, available to spend). See xref:chapters/13-advancement.adoc#chapter-advancement[Advancement, Talents, and Dark Secrets].
|===
...
| *Headquarters (HQ)* | The Covenant facility serving as the team's base of operations between Case Files. Provides medical care, equipment requisition via Stack during the Equipping Phase, research facilities, and downtime activities. See xref:chapters/12-headquarters.adoc#chapter-headquarters[Headquarters].
|===
