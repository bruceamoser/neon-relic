...
To reflect the covert nature of the Covenant, characters do not use personal cash. Instead, they utilize their *CL Pool* (equal to their Clearance Level) during the Equipping Phase to requisition supplies from Stack. Items are categorized by their required CL and count against the agent's CL Pool budget.
...
[cols="2,^1,^1,1,1,3,^1", options="header"]
|===
...
* During the *Equipping Phase* at the start of each Case File: any supply can be restocked to its starting die size. The supply item must be requisitioned through your CL Pool during the Equipping Phase (see <<requisitioning-gear>>) — the restock itself has no additional cost beyond the normal CL-pool requisition.
...
The table above describes who qualifies for each CL tier. The section below explains how CL translates into an equipment budget for each mission.
...
Only one above-CL roll is permitted per cell per Equipping Phase — each cell gets one shot at an over-budget item. Choose your over-budget item carefully.
